...
func makeRestrictedDialContext(cfg httpClientConfig, lggr *zap.Logger) func(context.Context, string, string) (net.Conn, error) {
	...
	return func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		if err == nil {
			...
			if restrict, rerr := isRestrictedIP(a.IP, cfg); rerr != nil {
				lggr.Error("Restricted IP check failed, this IP will be allowed", zap.Any("ip", a.IP), zap.Error(rerr))
			} else if restrict {
